angular.module('inspinia').controller('MainCtrl',
	function($scope, $rootScope, $state, Restangular, $http, $timeout ) {

		$rootScope.inspiniaTemplate = 'views/common/notify.html';
		
		var getMainInfo = function () {
			Restangular.one('/auth-users').get().then(function(response) {
				$scope.name = response.user.name;
				$scope.role = response.user.role;

				if ($scope.role != 'employee')
				{
					angular.element('.configurations').show();
				}
				if ($scope.role == 'super-admin')
				{
					angular.element('.users').show();
				}
			});

			Restangular.one('/locations').get().then(function(response) {
				$scope.locations = {};

				angular.forEach(response.locations, function (value, key) {
					$scope.locations[value.id] = value;
				})

				$timeout(function() {
					if (!localStorage.location) {
						localStorage.location = $scope.locations[Object.keys($scope.locations)[0]].id;
					} else {
						inArray = false;
					...
					$rootScope.selectedLocation = parseInt(localStorage.location);
				}, 20);
			});
		}
		...
		$rootScope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams, options) {
            if (toState.name != 'login' && (fromState.name == 'login' || fromState.name == '')) {
            	getMainInfo();
            }
        });
